                ...
                if 'IN_CLOSE_WRITE' in event_types:
                    ...
                    try:
                        ...
                        if self.is_artifacts:
                            self._report_artifact(remote_filename, self.crate,
                                                  self.runner)
                        else:
...
class FuzzProcess:
    ...
    def build(self) -> None:
        ...
        """
        print(
            f'Building fuzzer for branch {self.branch} and target {self.target}, log is at {self.log_path}'
        )
    ...
    def start(self, corpus: Corpus) -> None:
        ...
        print(
            f'Starting fuzzer for branch {self.branch} and target {self.target}, log is at {self.log_path}'
        )
    ...
    def report_crash(self, corpus: Corpus, bucket: gcs.Bucket) -> None:
        ...
        for l in log_lines[::-1]:
            if artifact_pattern in l:
                ...
                artifact = l.split(artifact_pattern)[1][:-1]
                break
...
def kill_fuzzers(bucket: gcs.Bucket,
    ...
    for f in fuzzers:
        try:
            ...
            print(f"Failed looking up process {f.proc.pid}")
    for f in fuzzers:
        bucket.blob(f"logs/{f.log_path}").upload_from_filename(
            str(LOGS_DIR / f.log_path))
...
def run_fuzzers(gcs_client: gcs.Client, pause_evt: threading.Event,
    ...
    while True:
        ...
        for (b, t) in zip(branches, targets):
            ...
            log_path = pathlib.Path(
                'fuzz') / date / t['crate'] / t['runner'] / str(uuid.uuid4())
            utils.mkdirs((LOGS_DIR / log_path).parent)
            log_file = LOGS_DIR / log_path
            fuzzers.append(
                FuzzProcess(corpus.version, b, t, worktree, log_path, log_file))
        ...
        while time.monotonic() < started + AUTO_REFRESH_INTERVAL_SECS:
            ...
            for f in fuzzers:
                if f.poll():
                    ...
                    worktree = repo.worktree(b['name'])
                    log_path = pathlib.Path(
                        'fuzz') / date / t['crate'] / t['runner'] / str(
                            uuid.uuid4())
                    utils.mkdirs((LOGS_DIR / log_path).parent)
                    log_file = LOGS_DIR / log_path
                    new_fuzzer = FuzzProcess(corpus.version, b, t, worktree,
                                             log_path, log_file)
            ...
            if time.monotonic() > next_sync_file_upload:
                ...
                for l in sync_log_files:
                    bucket.blob(f"logs/{l}").upload_from_filename(
                        str(LOGS_DIR / l))
...
def main() -> None:
    ...
    try:
        ...
        run_fuzzers(gcs, pause_event, resume_event,
